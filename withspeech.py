...
recognized_text = ""  # Store latest speech input
speech_chat_response = ""  # Store ChatGPT response from speech
...
# Speech recognition and ChatGPT response function
def speech_recognition_thread():
    global recognized_text, speech_chat_response
    ...
    with sr.Microphone() as source:
        ...
        while True:
            try:
                ...
                print("You said:", text)
                # Use original prompt as context, voice input as prompt
                context = "I am a blind person. You're responsible for my safety. Then, I'll send you an image every 5 seconds. You don't have to respond when there is nothing going on. However, if there's something, if there's information relevant to my safety, please send me a message."
                prompt = f"{context}\n\nUser voice input: {text}"
                response = client.chat.completions.create(
                    model="gpt-4o",
                    messages=[
                        {"role": "system", "content": "You are a helpful assistant for a blind person, providing safety-related guidance."},
                        {"role": "user", "content": prompt}
                    ],
                    max_tokens=300
                )
                speech_chat_response = response.choices[0].message.content.strip()
                print("ChatGPT Speech Response:", speech_chat_response)
            except sr.UnknownValueError:
                recognized_text = "Could not understand"
                speech_chat_response = "I couldn’t understand your speech. Please try again."
            except sr.RequestError as e:
                recognized_text = f"Error: {e}"
                speech_chat_response = f"Speech recognition error: {e}"
            except Exception as e:
                speech_chat_response = f"ChatGPT Error: {e}"
...
current_analysis = ""  # Store latest ChatGPT message from frame analysis
...
while True:
    ...
    # Display speech input and ChatGPT speech response
    cv2.putText(frame, f"Speech: {recognized_text[:30]}", (10, 180),  # Truncate for display
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
    if speech_chat_response:
        cv2.putText(frame, f"ChatGPT: {speech_chat_response[:50]}", (10, 210),  # Truncate for display
